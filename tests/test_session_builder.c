...
START_TEST(test_basic_key_exchange)
{
    ...
    signal_buffer *alice_key_exchange_message_serialized =
    ...
    result = key_exchange_message_deserialize(&alice_key_exchange_message_deserialized,
            signal_buffer_data(alice_key_exchange_message_serialized),
            signal_buffer_len(alice_key_exchange_message_serialized), global_context);
    ck_assert_int_eq(result, 0);
    ...
    signal_buffer *bob_key_exchange_message_serialized =
    ...
    result = key_exchange_message_deserialize(&bob_key_exchange_message_deserialized,
            signal_buffer_data(bob_key_exchange_message_serialized),
            signal_buffer_len(bob_key_exchange_message_serialized), global_context);
    ck_assert_int_eq(result, 0);
    ...
    ck_assert_int_eq(signal_protocol_session_contains_session(alice_store, &bob_address), 1);
    ck_assert_int_eq(signal_protocol_session_contains_session(bob_store, &alice_address), 1);
    ...
    alice_store = 0;
    SIGNAL_UNREF(alice_key_exchange_message);
    SIGNAL_UNREF(bob_key_exchange_message);
    ...
    SIGNAL_UNREF(temp_key_exchange_message);

    run_interaction(alice_store, bob_store);

    /* Cleanup */
    SIGNAL_UNREF(response);
    SIGNAL_UNREF(bob_key_exchange_message_deserialized);
    SIGNAL_UNREF(bob_key_exchange_message);
    SIGNAL_UNREF(alice_key_exchange_message_deserialized);
    SIGNAL_UNREF(alice_key_exchange_message);
    session_builder_free(alice_session_builder);
...
START_TEST(test_simultaneous_key_exchange)
{
    ...
    /* Cleanup */
    SIGNAL_UNREF(alice_ack);
    SIGNAL_UNREF(bob_ack);
    SIGNAL_UNREF(alice_response);
    SIGNAL_UNREF(bob_response);
    SIGNAL_UNREF(alice_key_exchange);
    SIGNAL_UNREF(bob_key_exchange);
    session_builder_free(alice_session_builder);
    session_builder_free(bob_session_builder);
    signal_protocol_store_context_destroy(alice_store);
    signal_protocol_store_context_destroy(bob_store);
